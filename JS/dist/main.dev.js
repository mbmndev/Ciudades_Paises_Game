...
var _questions = require("../DATOS/questions.js");

var contadorjuego = document.getElementById("contador");
var cont = contadorjuego.value; //MAPAS

var mymap;
var marcador; //TIEMPO GRAFICA 

var ArrayTiempo = [['Ejemplo', 'Tiempo']]; //VECES PAIS

var ArrayPizza = [['Queso', 'Veces que aparece']]; //DESORDENAR 

function Desordenar(DatosPaises) {
  var paisesDesordenados = [];
  var DatosDesordenados = DatosPaises.sort(function () {
...
var url = '../DATOS/questions.js'; //BOTON NUEVA PARTIDA

var botonewgame = document.getElementById('NuevaPartida');
botonewgame.addEventListener('click', function () {
  //MAPA EN CESAR
  mymap.flyTo(coordenadas, 13), L.marker(coordenadas).addTo(mymap); //DESACTIVA BOTON

  cont = "0";
  botonewgame.setAttribute('disabled', "true"); //DESTRUCTOR

  destructor(); //EMPIEZA EL CONTADOR

  var intevalo = setInterval(contador, 1000);
  var aciertos = 0; //DESOREDENAR VALORES

  var num = AleatorioTres();
  Desordenar(_questions.gameData.countries);
  var DatosDesordenados = Desordenar(_questions.gameData.countries);
  var datosPaises = Desordenar(DatosDesordenados);
  constructorCiudades(DatosDesordenados, num);
  constructorPaises(datosPaises); //HACER QUE SEAN DRAGABLES Y DROPABLES
  ...
  $(function () {
    ...
    $(".cuerpopaises").droppable({
      drop: function drop(event, ui) {
        ...
        aciertos++;
        pasarAPizza(event.target.parentNode.firstElementChild.textContent); //PASAR A GRAFICO PIZZA
        ...
        if (aciertos == 5) {
          ...
          google.charts.setOnLoadCallback(drawChart1); //HABILITAR BOTON
...
function constructorCiudades(datos, num) {
  ...
  var redesorden = datospasados.sort(function () {
    return Math.random() - 0.5;
  });
  var contadordivs = 0;
  redesorden.forEach(function (pais) {
    if (contadordivs < 5) {
      var divPaises = document.getElementById('DivCiudades');
      var nuevodiv = document.createElement('div');
      var titulo = document.createElement('spam'); //ESTRUCTURA
...
function constructorPaises(datos) {
  datos.forEach(function (pais) {
    ...
    var divresto = document.createElement('div'); //ESTRUCTURA
    ...
    $("#".concat(pais.code)).droppable({
      accept: "#city".concat(pais.code)
    });
  });
} //DESTRUCTOR
...
var coordenadas = [28.455970688984355, -16.28325747316448]; //ONLOAD
...
function movimientomapa(ciudad) {
  _questions.gameData.countries.forEach(function (pais) {
    pais.cities.forEach(function (city) {
      if (city.name == ciudad) {
        mymap.flyTo(city.location, 13), L.marker(city.location).addTo(mymap);
      }
    });
  });
} //CHART 1
...
function pasarAPizza(nombrePais) {
  var comprobar = false;
  ArrayPizza.forEach(function (element) {
